...
/**
 * Provisiona el resto del esquema (todas las tablas salvo `services`, que crea
 * la migración baseline InitServices) y siembra los datos iniciales. Idempotente:
 * usa CREATE TABLE IF NOT EXISTS e INSERT IGNORE, por lo que en bases ya
 * existentes (local/develop) es un no-op y en una base nueva (producción) crea y
 * puebla todo. Necesaria porque se retiró la degradación a memoria de los repos.
 */
export class SchemaAndSeed1786300000000 implements MigrationInterface {
  name = 'SchemaAndSeed1786300000000';
  ...
  public async up(q: QueryRunner): Promise<void> {
    ...
    await q.query(`CREATE TABLE IF NOT EXISTS site_config (
      config_key VARCHAR(80) PRIMARY KEY,
      config_value TEXT NULL,
      updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
    ) ${ENGINE}`);

    await q.query(`CREATE TABLE IF NOT EXISTS blog_posts (
      id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    await q.query(`CREATE TABLE IF NOT EXISTS plans (
      id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    await q.query(`CREATE TABLE IF NOT EXISTS project_requests (
      id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    // ── Semillas (INSERT IGNORE: no duplica si ya existen) ──────────────────
    await q.query(`INSERT IGNORE INTO services (id, slug, title, description, icon, accent_color, tags, is_featured) VALUES
    ...
    await q.query(`INSERT IGNORE INTO plans (id, slug, name, tagline, price_monthly, price_monthly_gtq, currency, features, services, accent_color, cta_label, is_popular) VALUES
    ...
    await q.query(`INSERT IGNORE INTO blog_posts (id, slug, title, excerpt, content, category, author, reading_time, status, published_at) VALUES
  ...
  public async down(q: QueryRunner): Promise<void> {
    ...
    await q.query('DROP TABLE IF EXISTS project_requests');
    await q.query('DROP TABLE IF EXISTS plans');
    await q.query('DROP TABLE IF EXISTS blog_posts');
    await q.query('DROP TABLE IF EXISTS site_config');
